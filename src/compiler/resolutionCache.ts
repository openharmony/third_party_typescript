...
import {
    ...
    isOHModulesAtTypesDirectory, isOhpm, isRootedDiskPath, isString, isTargetModulesDerectory, isTraceEnabled, length,
    loadModuleFromGlobalCache, Map, memoize, MinimalResolutionCacheHost, ModeAwareCache, ModuleKind,
    ModuleResolutionCache, ModuleResolutionHost, mutateMap, noopFileWatcher, normalizePath, PackageId,
    packageIdToString, parseModuleFromPath, Path, pathContainsNodeModules, pathContainsOHModules, PerModuleNameCache,
    Program, ReadonlyESMap, removeSuffix, removeTrailingDirectorySeparator, resolutionExtensionIsTSOrJson,
    ResolvedModuleFull, ResolvedModuleWithFailedLookupLocations, ResolvedProjectReference,
    ResolvedTypeReferenceDirective, ResolvedTypeReferenceDirectiveWithFailedLookupLocations, returnTrue, Set, some,
    SourceFile, startsWith, stringContains, trace, unorderedRemoveItem, WatchDirectoryFlags,
} from "./_namespaces/ts";
...
export function createResolutionCache(resolutionHost: ResolutionCacheHost, rootDirForResolution: string | undefined, logChangesWhenResolvingModule: boolean): ResolutionCache {
     ...
     */
    const failedLookupDefaultExtensions = resolutionHost.getCompilationSettings().ets
        ? [Extension.Ets, Extension.Ts, Extension.Tsx, Extension.Js, Extension.Jsx, Extension.Json]
        : [Extension.Ts, Extension.Tsx, Extension.Js, Extension.Jsx, Extension.Json, Extension.Ets];
    ...
    function getDirectoryToWatchFromFailedLookupLocationDirectory(dir: string, dirPath: Path): DirectoryOfFailedLookupWatch | undefined {
        ...
        if (isTargetModulesDerectory(dirPath)) {
    ...
    function scheduleInvalidateResolutionOfFailedLookupLocation(fileOrDirectoryPath: Path, isCreatingWatchedDirectory: boolean) {
        if (isCreatingWatchedDirectory) {
            ...
            if (isNodeModulesAtTypesDirectory(fileOrDirectoryPath) || (isOHModules && isOHModulesAtTypesDirectory(fileOrDirectoryPath)) ||
                ...
                const packagePath = parseModuleFromPath(fileOrDirectoryPath);
