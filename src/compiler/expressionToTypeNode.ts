...
export function createSyntacticTypeNodeBuilder(options: CompilerOptions, resolver: SyntacticTypeNodeBuilderResolver) {
    ...
    function serializeExistingTypeAnnotation(type: TypeNode | undefined): boolean | undefined {
        return type === undefined ?
            undefined : !type.parent || !isParameter(type.parent) || !resolver.requiresAddingImplicitUndefined(type.parent) || canAddUndefined(type);
    }
    function serializeTypeOfExpression(
        expr: Expression,
    ...
    function serializeTypeOfDeclaration(node: HasInferredType, context: SyntacticTypeNodeBuilderContext): boolean | undefined {
        switch (node.kind) {
    ...
    function getTypeAnnotationFromAccessor(accessor: AccessorDeclaration): TypeNode | undefined {
        ...
        if (accessor.kind === SyntaxKind.GetAccessor) {
            return getEffectiveReturnTypeNode(accessor); // Getter - return type
        }
        if (accessor.parameters.length > 0) {
            return getEffectiveTypeAnnotationNode(accessor.parameters[0]); // Setter parameter type
        }
        return undefined;
    }
    function getTypeAnnotationFromAllAccessorDeclarations(node: AccessorDeclaration, accessors: AllAccessorDeclarations): TypeNode | undefined {
    ...
    function typeFromAccessor(node: AccessorDeclaration, context: SyntacticTypeNodeBuilderContext): boolean | undefined {
    ...
    function typeFromVariable(node: VariableDeclaration, context: SyntacticTypeNodeBuilderContext): boolean | undefined {
        ...
        if (node.initializer && !resolver.isExpandoFunctionDeclaration(node)) {
            resultType = typeFromExpression(
                node.initializer, context, /*isConstContext*/ undefined, /*requiresAddingUndefined*/ undefined, isVarConst(node)
            );
        }
        return resultType ?? inferTypeOfDeclaration(node, context);
    }
    function typeFromParameter(node: ParameterDeclaration, context: SyntacticTypeNodeBuilderContext): boolean | undefined {
    ...
    function typeFromProperty(node: PropertyDeclaration, context: SyntacticTypeNodeBuilderContext): boolean | undefined {
        const declaredType = getEffectiveTypeAnnotationNode(node);
        if (declaredType) {
            return serializeExistingTypeAnnotation(declaredType);
        }
    ...
    function inferExpressionType(node: Expression, context: SyntacticTypeNodeBuilderContext): boolean {
        context.tracker.reportInferenceFallback(node);
        return false;
    }

    function inferReturnTypeOfSignatureSignature(node: SignatureDeclaration | JSDocSignature, context: SyntacticTypeNodeBuilderContext): boolean {
        context.tracker.reportInferenceFallback(node);
        return false;
    }

    function inferAccessorType(node: GetAccessorDeclaration | SetAccessorDeclaration, context: SyntacticTypeNodeBuilderContext): boolean {
    ...
    function typeFromTypeAssertion(expression: Expression, type: TypeNode, context: SyntacticTypeNodeBuilderContext, requiresAddingUndefined: boolean): boolean | undefined {
    ...
    function typeFromExpression(
        ...
        requiresAddingUndefined: boolean = false,
        preserveLiterals: boolean = false
    ): boolean | undefined {
        switch (node.kind) {
    ...
    function typeFromFunctionLikeExpression(fnNode: FunctionExpression | ArrowFunction, context: SyntacticTypeNodeBuilderContext): boolean {
    ...
    function canGetTypeFromArrayLiteral(arrayLiteral: ArrayLiteralExpression, context: SyntacticTypeNodeBuilderContext, isConstContext: boolean): boolean {
        if (!isConstContext) {
    ...
    function typeFromArrayLiteral(arrayLiteral: ArrayLiteralExpression, context: SyntacticTypeNodeBuilderContext, isConstContext: boolean): boolean {
    ...
    function canGetTypeFromObjectLiteral(objectLiteral: ObjectLiteralExpression, context: SyntacticTypeNodeBuilderContext): boolean {
        let result = true;
    ...
    function typeFromObjectLiteral(objectLiteral: ObjectLiteralExpression, context: SyntacticTypeNodeBuilderContext, isConstContext: boolean): boolean {
        ...
        for (const prop of objectLiteral.properties) {
            ...
            if (prop.name.kind === SyntaxKind.ComputedPropertyName) {
                if (!resolver.isNonNarrowedBindableName(prop.name)) {
                    ...
                    const visibilityResult = resolver.isEntityNameVisible(
                        prop.name.expression, context.enclosingDeclaration!, /*shouldComputeAliasToMakeVisible*/ false
                    );
            ...
            switch (prop.kind) {
                case SyntaxKind.MethodDeclaration:
                    canInferObjectLiteral = !!typeFromObjectLiteralMethod(prop, context) && canInferObjectLiteral;
                    break;
                case SyntaxKind.PropertyAssignment:
                    canInferObjectLiteral = !!typeFromObjectLiteralPropertyAssignment(prop, context, isConstContext) && canInferObjectLiteral;
                    break;
                case SyntaxKind.SetAccessor:
                case SyntaxKind.GetAccessor:
                    canInferObjectLiteral = !!typeFromObjectLiteralAccessor(prop, context) && canInferObjectLiteral;
                    break;
            }
        }
    ...
    function typeFromObjectLiteralPropertyAssignment(prop: PropertyAssignment, context: SyntacticTypeNodeBuilderContext, isConstContext: boolean) {
        return typeFromExpression(prop.initializer, context, isConstContext) ?? inferTypeOfDeclaration(prop, context);
    }

    function ensureParameter(p: ParameterDeclaration, context: SyntacticTypeNodeBuilderContext): boolean | undefined {
        return typeFromParameter(p, context);
    }

    function reuseTypeParameters(typeParameters: NodeArray<TypeParameterDeclaration> | undefined): boolean {
    ...
    function typeFromObjectLiteralMethod(method: MethodDeclaration, context: SyntacticTypeNodeBuilderContext): boolean {
    ...
    function typeFromObjectLiteralAccessor(
    ...
    ): boolean | undefined
    {
        ...
        if (getAccessorType !== undefined && setAccessorType !== undefined) {
            ...
            const propertyType = foundType ? serializeExistingTypeAnnotation(foundType) : inferAccessorType(accessor, context);
    ...
    function typeFromPrimitiveLiteral(): boolean {
        return true;
    }
    ...
    function createReturnFromSignature(fn: SignatureDeclaration | JSDocSignature, context: SyntacticTypeNodeBuilderContext): boolean {
        let returnType;
    ...
    function typeFromSingleReturnExpression(
        declaration: FunctionLikeDeclaration | undefined,
        context: SyntacticTypeNodeBuilderContext
    ): boolean | undefined
    {
