import {
    ...
    getOwnKeys, getPackageJsonByPMType, getPackageJsonTypesVersionsPaths, getPackageNameFromTypesPackageName,
    getPathsBasePath, getRelativePathFromDirectory, getRelativePathToDirectoryOrUrl, getSourceFileOfModule,
    getSupportedExtensions, getTextOfIdentifierOrLiteral, hasJSFileExtension, hasTSFileExtension,
    hostGetCanonicalFileName, Identifier, isAmbientModule, isApplicableVersionedTypesKey, isExternalModuleAugmentation,
    isExternalModuleNameRelative, isModuleBlock, isModuleDeclaration, isNonGlobalAmbientModule, isOhpm,
    isRootedDiskPath, isSourceFile, isString, JsxEmit, map, Map, mapDefined, MapLike, matchPatternOrExact, min,
    ModuleDeclaration, ModuleKind, ModulePath, ModuleResolutionHost, ModuleResolutionKind, ModuleSpecifierCache,
    ModuleSpecifierOptions, ModuleSpecifierResolutionHost, NodeFlags, NodeModulePathParts, nodeModulesPathPart,
    normalizePath, ohModulesPathPart, Path, pathContainsNodeModules, pathContainsOHModules, pathIsBareSpecifier,
    pathIsRelative, PropertyAccessExpression, removeFileExtension, removeSuffix, resolvePath, ScriptKind, some,
    SourceFile, startsWith, startsWithDirectory, stringContains, StringLiteral, Symbol, SymbolFlags, toPath,
    tryGetExtensionFromPath, tryParsePatterns, TypeChecker, UserPreferences,
} from "./_namespaces/ts";
...
export function getNodeModulesPackageName(
    ...
    const modulePaths = getAllModulePaths(importingSourceFile.path, nodeModulesFileName, host, preferences, options, compilerOptions.packageManagerType);
...
function getModuleSpecifierWorker(
    ...
    const modulePaths = getAllModulePaths(importingSourceFileName, toFileName, host, userPreferences, options, compilerOptions.packageManagerType);
...
function getAllModulePaths(
    ...
    const modulePaths = getAllModulePathsWorker(importingFilePath, importedFileName, host, packageManagerType);
    if (cache) {
...
function getAllModulePathsWorker(importingFileName: Path, importedFileName: string, host: ModuleSpecifierResolutionHost, packageManagerType?: string): readonly ModulePath[] {
    const getCanonicalFileName = hostGetCanonicalFileName(host);
...
function tryGetModuleNameAsNodeModule({ path, isRedirect }: ModulePath, { getCanonicalFileName, sourceDirectory }: Info, importingSourceFile: SourceFile , host: ModuleSpecifierResolutionHost, options: CompilerOptions, userPreferences: UserPreferences, packageNameOnly?: boolean, overrideMode?: ModuleKind.ESNext | ModuleKind.CommonJS): string | undefined {
    ...
    const parts: NodeModulePathParts = getNodeModulePathParts(path, isOhpm(options.packageManagerType) ? ohModulesPathPart : nodeModulesPathPart)!;
    if (!parts) {
        return undefined;
    }
