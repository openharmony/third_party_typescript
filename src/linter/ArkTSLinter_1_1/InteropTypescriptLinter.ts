...
export class InteropTypescriptLinter {
  ...
  private checkClassOrInterfaceDeclarationHeritageClause(hClause: HeritageClause): void {
    for (const tsTypeExpr of hClause.types) {
      ...
      */
      const tsExprType = reduceReference(InteropTypescriptLinter.tsTypeChecker.getTypeAtLocationForLinter(tsTypeExpr));
  ...
  private handleSendableGenericTypes(node: NewExpression): void {
    const type = InteropTypescriptLinter.tsTypeChecker.getTypeAtLocationForLinter(node);
  ...
  private handleAsExpression(node: Node): void {
    ...
    const targetType = InteropTypescriptLinter.tsTypeChecker.getTypeAtLocationForLinter(tsAsExpr.type).getNonNullableType();
    const exprType = InteropTypescriptLinter.tsTypeChecker.getTypeAtLocationForLinter(tsAsExpr.expression).getNonNullableType();
