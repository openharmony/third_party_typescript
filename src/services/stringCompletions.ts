import {
    ...
    CompilerOptions, CompletionEntry, CompletionEntryDetails, CompletionInfo, contains, containsPath, ContextFlags,
    createSortedArray, createTextSpan, createTextSpanFromStringLiteralLikeContent, Debug, deduplicate,
    directorySeparator, ElementAccessExpression, emptyArray, endsWith, ensureTrailingDirectorySeparator,
    equateStringsCaseSensitive, Extension, fileExtensionIsOneOf, filter, find, findAncestor, findPackageJson,
    findPackageJsons, firstDefined, firstOrUndefined, flatMap, flatten, forEachAncestorDirectory, getBaseFileName,
    getContextualTypeFromParent, getDirectoryPath, getEffectiveTypeRoots, getEmitModuleResolutionKind,
    getLeadingCommentRanges, getModeForUsageLocation, getModuleByPMType, getOwnKeys, getPackageJsonTypesVersionsPaths,
    getPathComponents, getReplacementSpanForContextToken, getSupportedExtensions,
    getSupportedExtensionsWithJsonIfResolveJsonModule, getTokenAtPosition, hasIndexSignature, hasProperty,
    hasTrailingDirectorySeparator, hostGetCanonicalFileName, IndexedAccessTypeNode, isApplicableVersionedTypesKey,
    isArray, isCallExpression, isIdentifier, isIdentifierText, isImportCall, isInReferenceComment, isInString,
    isJsxAttribute, isJsxOpeningLikeElement, isLiteralTypeNode, isObjectLiteralExpression, isOhpm, isPatternMatch,
    isPrivateIdentifierClassElementDeclaration, isRootedDiskPath, isString, isStringLiteral, isStringLiteralLike,
    isTypeReferenceNode, isUrl, Iterator, JsxAttribute, LanguageServiceHost, length, LiteralExpression, LiteralTypeNode,
    Map, mapDefined, MapLike, ModuleKind, ModuleResolutionKind, moduleSpecifiers, Node, normalizePath, normalizeSlashes,
    ObjectLiteralExpression, Path, Program, PropertyAssignment, rangeContainsPosition, readJson, removeFileExtension,
    removePrefix, removeTrailingDirectorySeparator, resolvePath, ScriptElementKind, ScriptElementKindModifier,
    ScriptTarget, Signature, signatureHasRestParameter, SignatureHelp, singleElementArray, skipConstraint,
    skipParentheses, SourceFile, startsWith, stringContains, StringLiteralLike, StringLiteralType, stripQuotes, Symbol,
    SyntaxKind, textPart, TextSpan, tryAndIgnoreErrors, tryDirectoryExists, tryFileExists, tryGetDirectories,
    tryGetExtensionFromPath, tryParsePattern, tryReadDirectory, tryRemoveDirectoryPrefix, tryRemovePrefix, Type,
    TypeChecker, TypeFlags, UnionTypeNode, unmangleScopedPackageName, UserPreferences, walkUpParenthesizedExpressions,
...
function kindModifiersFromExtension(extension: Extension | undefined): ScriptElementKindModifier {
    switch (extension) {
        ...
        case Extension.Dets: return ScriptElementKindModifier.detsModifier;
    ...
    if (isEmitResolutionKindUsingNodeModules(compilerOptions)) {
        ...
        if (!foundGlobal) {
            let ancestorLookup: (directory: string) => void | undefined = ancestor => {
                const nodeModules = combinePaths(ancestor, getModuleByPMType(host.getCompilationSettings().packageManagerType));
