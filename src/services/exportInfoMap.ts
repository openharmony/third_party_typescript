import {
    ...
    GetCanonicalFileName, getDirectoryPath, getLocalSymbolForExportDefault, getModuleByPMType, getNameForExportedSymbol,
    ...
    isExternalOrCommonJsModule, isIdentifier, isKnownSymbol, isNonGlobalAmbientModule, isOhpm,
    isPrivateIdentifierSymbol, LanguageServiceHost, Map, mapDefined, ModuleSpecifierCache,
    ModuleSpecifierResolutionHost, moduleSpecifiers, nodeModulesPathPart, ohModulesPathPart, PackageJsonImportFilter,
    Path, Program, skipAlias, skipOuterExpressions, SourceFile, startsWith, Statement, stringContains, stripQuotes,
    Symbol, SymbolFlags, timestamp, tryCast, TypeChecker, unescapeLeadingUnderscores, unmangleScopedPackageName,
    UserPreferences,
} from "./_namespaces/ts";
...
export function createCacheableExportInfoMap(host: CacheableExportInfoMapHost): ExportInfoMap {
    ...
    const cache: ExportInfoMap = {
        ...
        add: (importingFile, symbol, symbolTableKey, moduleSymbol, moduleFile, exportKind, isFromPackageJson, checker) => {
            ...
            let packageName;
            if (moduleFile) {
                const nodeModulesPathParts = getNodeModulePathParts(moduleFile.fileName, isOhpm(host.getCurrentProgram()?.getCompilerOptions().packageManagerType) ? ohModulesPathPart : nodeModulesPathPart);
    ...
    const hasImportablePath = !!moduleSpecifiers.forEachFileNameOfModule(
        ...
        toPath => {
            ...
            return (toFile === to || !toFile) &&
                isImportablePath(from.fileName, toPath, getCanonicalFileName, globalTypingsCache, program.getCompilerOptions().packageManagerType);
        }
    );
 ...
 */
function isImportablePath(fromPath: string, toPath: string, getCanonicalFileName: GetCanonicalFileName, globalCachePath?: string, packageManagerType?: string): boolean {
    // If it's in a `node_modules` but is not reachable from here via a global import, don't bother.
    const toNodeModules = forEachAncestorDirectory(toPath, ancestor => (getBaseFileName(ancestor) === getModuleByPMType(packageManagerType)) ? ancestor : undefined);
