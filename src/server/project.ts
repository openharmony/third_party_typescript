...
import {
    ...
    CompilerHost, CompilerOptions, concatenate, ConditionCheckResult, ConfigFileProgramReloadLevel,
    createCacheableExportInfoMap, createLanguageService, createResolutionCache, createSymlinkCache, Debug, Diagnostic,
    DiagnosticCategory, DirectoryStructureHost, DirectoryWatcherCallback, DocumentPositionMapper, DocumentRegistry,
    enumerateInsertsAndDeletes, ESMap, every, explainFiles, ExportInfoMap, Extension, FileCheckModuleInfo,
    fileExtensionIs, FileReference, FileWatcher, FileWatcherCallback, FileWatcherEventKind, filter, firstDefined,
    flatMap, forEach, forEachEntry, forEachKey, generateDjb2Hash, getAllowJSCompilerOption,
    getAutomaticTypeDirectiveNames, GetCanonicalFileName, getDeclarationEmitOutputFilePathWorker,
    getDefaultCompilerOptions, getDefaultLibFileName, getDefaultLibFilePath, getDirectoryPath, getEffectiveTypeRoots,
    getEmitDeclarations, getEntrypointsFromPackageJsonInfo, getNormalizedAbsolutePath, getOrUpdate, getStringComparer,
    HasInvalidatedResolutions, HostCancellationToken, inferredTypesContainingFile, InstallPackageOptions,
    IScriptSnapshot, isDeclarationFileName, isExternalModuleNameRelative, isInsideNodeModules, isOhpm, JSDoc,
    JsDocNodeCheckConfig, JsDocTagInfo, JsTyping, LanguageService, LanguageServiceHost, LanguageServiceMode, map, Map,
    mapDefined, maybeBind, ModuleKind, ModuleResolutionCache, ModuleResolutionHost, noop, noopFileWatcher,
    normalizePath, normalizeSlashes, orderedRemoveItem, outFile, PackageJsonAutoImportPreference, PackageJsonInfo,
    ParsedCommandLine, parsePackageName, Path, perfLogger, PerformanceEvent, PluginImport, PollingInterval, Program,
    ProjectPackageJsonInfo, ProjectReference, removeFileExtension, ResolutionCache, resolutionExtensionIsTSOrJson,
...
export function hasNoTypeScriptSource(fileNames: string[]): boolean {
    return !fileNames.some(fileName => (fileExtensionIs(fileName, Extension.Ts) && !isDeclarationFileName(fileName)) || fileExtensionIs(fileName, Extension.Tsx) || (fileExtensionIs(fileName, Extension.Ets) && !isDeclarationFileName(fileName)));
}
...
export abstract class Project implements LanguageServiceHost, ModuleResolutionHost {
    ...
    constructor(
        ...
        if (this.projectService.host.getJsDocNodeConditionCheckedResult) {
            this.getJsDocNodeConditionCheckedResult = this.projectService.host.getJsDocNodeConditionCheckedResult;
        }
    ...
    getSymlinkCache(): SymlinkCache {
        if (!this.symlinks) {
            this.symlinks = createSymlinkCache(this.getCurrentDirectory(), this.getCanonicalFileName, isOhpm(this.compilerOptions.packageManagerType));
        }
