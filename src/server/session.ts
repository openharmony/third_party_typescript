import {
    ...
    getTextOfJSDocTagInfo, getTouchingPropertyName, GoToDefinition, HostCancellationToken, identity,
    ImplementationLocation, ImportSpecifier, isAccessExpression, isArray, isDeclarationFileName, isIdentifier, isOhpm,
    isString, isStringLiteralLike, JSDocLinkDisplayPart, JSDocTagInfo, LanguageServiceMode, LineAndCharacter, map, Map,
    mapDefined, mapDefinedIterator, mapIterator, mapOneOrMany, memoize, ModuleResolutionKind, MultiMap, NavigateToItem,
    NavigationBarItem, NavigationTree, nodeModulesPathPart, normalizePath, ohModulesPathPart,
    OperationCanceledException, OrganizeImportsMode, outFile, OutliningSpan, Path, perfLogger, PerformanceEvent,
    PossibleProgramFileInfo, Program, QuickInfo, RefactorEditInfo, ReferencedSymbol, ReferencedSymbolDefinitionInfo,
    ReferencedSymbolEntry, ReferenceEntry, removeFileExtension, RenameInfo, RenameLocation, ScriptKind, SelectionRange,
    SemanticClassificationFormat, Set, SignatureHelpItem, SignatureHelpItems, singleIterator, some, SourceFile,
    startsWith, stringContains, SymbolDisplayPart, SyntaxKind, TextChange, TextInsertion, TextRange, TextSpan,
    textSpanEnd, toArray, toFileNameLowerCase, tracing, unmangleScopedPackageName, UserPreferences, version,
    WithMetadata,
} from "./_namespaces/ts";
import {
...
export class Session<TMessage = string> implements EventSender {
    ...
    private findSourceDefinition(args: protocol.FileLocationRequestArgs): readonly protocol.DefinitionInfo[] {
        ...
        function findImplementationFileFromDtsFileName(fileName: string, resolveFromFile: string, auxiliaryProject: Project) {
            const nodeModulesPathParts = getNodeModulePathParts(fileName,  isOhpm(project.getCompilationSettings().packageManagerType) ? ohModulesPathPart : nodeModulesPathPart);
    ...
    private mapJSDocTagInfo(tags: JSDocTagInfo[] | undefined, project: Project, richResponse: boolean): protocol.JSDocTagInfo[] {
        return tags ? tags.map(tag => ({
            ...tag,
            text: richResponse && Array.isArray(tag.text) ? this.mapDisplayParts(tag.text, project): getTextOfJSDocTagInfo(tag)
        })) : [];
    }
